...
private class TitleView : UIView {
    ...
    private func setup() {
        ...
        /// 按钮
        editButton.setTitle(LanguageKey.Common_Edit.value, for: .normal)
        ...
        titleLabel.text = LanguageKey.MyMusic.value
    ...
    @objc private func onEditButtonClicked(sender:UIButton) {
        ...
        if sender.currentTitle! == LanguageKey.Common_Edit.value {
            editButton.setTitle(LanguageKey.Common_Done.value, for: .normal)
            type = .finished
        } else {
            editButton.setTitle(LanguageKey.Common_Edit.value, for: .normal)
        }
